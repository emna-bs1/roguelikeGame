...
obstacles = ['basic/PNG/Props/Tree.png',
             'basic/PNG/Props/Stump1.png',
             'basic/PNG/Props/Log1.png',
            'basic/PNG/Props/Well.png',
            'basic/PNG/Props/Tree_07.png',
            'basic/PNG/Props/Tree_03.png',
            'basic/PNG/Props/Rock_02.png',
            'basic/PNG/Props/Tire_03.png']
...
walls = []
floors = []
sprite_walls = pygame.sprite.Group()
sprite_floors = pygame.sprite.Group()
...
def map_size():
    wall_x = set(map(lambda x: x.rect[0], walls))
    max_x = max(wall_x)
    wall_y = set(map(lambda x: x.rect[1], walls))
    max_y = max(wall_y)
    return  max_x, max_y
...
class Wall(pygame.sprite.Sprite):
    def __init__(self,pos):
        ...
        self.image = pygame.image.load('basic/PNG/Blocks/wall.png')
        self.rect = self.image.get_rect()
        self.rect.x = pos[0]
        self.rect.y = pos[1]
        self.collide = self.rect

        self.guide_rect = pygame.Rect(self.rect.x / (16) , (self.rect.y) / (16) , self.rect.width / 16  ,self.rect.height / 16 )

        walls.append(self)
        sprite_walls.add(self)
        self.define_pos(pos)
...
class Floor(pygame.sprite.Sprite):
    def __init__(self, pos, image ='basic/PNG/Tiles/floor1.png'):
        pygame.sprite.Sprite.__init__(self)
        ...
        floors.append(self)
        sprite_floors.add(self)
        self.define_pos(pos)

    def define_pos(self, pos):
        floor_pos.append(pos)

class Background(Floor):
    def __init__(self,pos):
        super().__init__(pos)
        sprite_floors.add(self)
        floors.remove(self)
    ...
    def define_pos(self, pos):
        background_pos.append(pos)

class Obstacle(Wall):
    def __init__(self,pos):
        Wall.__init__(self,pos)
        self.image = pygame.image.load(obstacles[random.randint(0,len(obstacles)-1)])
        self.rect = self.image.get_rect()
        self.rect.x = pos[0]
        self.rect.y = pos[1]
        self.collide.width = self.rect.width * 2 / 3
        self.collide.height= self.rect.height * 2 / 3
        self.collide.center = self.rect.center
        self.guide_rect = pygame.Rect(self.rect.x / 16 , (self.rect.y) / 16 , self.rect.width / 16 - 1 ,self.rect.height / 16 - 1)
...
def generate_hero_initial_pos(screen_width,screen_height):
    while 1:
        initial_pos = floors[random.randint(0, len(floors) - 1)]
        return initial_pos.rect.centerx, initial_pos.rect.centery

class Generator():
    def __init__(self, width=64, height=64, max_rooms=15, min_room_xy=7,
                 max_room_xy=20, rooms_overlap=False, random_connections=10,
                 random_spurs=10, tiles=CHARACTER_TILES):
        self.width = width
        self.height = height
    ...
    def parse_level(self):
        ...
        for row in self.tiles_level:
            for col in row:
                ...
                if col == ".":
                    Floor((x, y))
                    if random.randint(0, floors_sum // 200) == floors_sum // 200:
                        Obstacle((x, y))
                    elif random.randint(0, floors_sum // 500) == floors_sum // 500 and not done:
